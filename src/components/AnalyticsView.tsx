...
export default function AnalyticsView({ theme, refreshTrigger }: Props) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const data = await Promise.resolve(loadBondScoreRecords());
      setRecords(data);
  ...
  // Prepare chart data (last 14 records for bar chart - 2 weeks)
  const chartRecords = records.slice(0, 14).reverse();
  const chartScores = chartRecords.map(r => r.bondScore);
  const maxScore = Math.max(...(chartScores.length > 0 ? chartScores : [100]), 100);
      ...
      {chartScores.length > 0 && (
        ...
        >
          ...
          <View style={styles.chartContainer}>
            {/* Simple Bar Chart (like Dashboard) */}
            <View style={{ flexDirection: "row", alignItems: "flex-end", height: 150, marginBottom: 8 }}>
              {chartScores.map((score, index) => {
                const barHeight = Math.max(8, Math.round((score / maxScore) * 130));
                    ...
                    <LinearGradient
                      colors={
                        isRecent
                          ...
                          : [theme.textMuted + "80", theme.textMuted + "60"]
                      }
                      start={{ x: 0, y: 1 }}
                      end={{ x: 0, y: 0 }}
                      style={{
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 8 }}>
              {chartScores.map((_, index) => {
                if (index % Math.ceil(chartScores.length / 7) !== 0 && index !== chartScores.length - 1) {
                  return <View key={index} style={{ flex: 1 }} />;
                }
                return (
                  <Text key={index} style={{ fontSize: 10, color: theme.textMuted, flex: 1, textAlign: "center" }}>
                    {index + 1}
                  </Text>
                );
              })}
            ...
            <View style={{ marginTop: 8, paddingHorizontal: 8 }}>
              <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                ...
                <Text style={{ fontSize: 11, color: theme.textMuted }}>Score: {Math.round(maxScore)}</Text>
              </View>
            </View>
          </View>
        </View>
